/**
 * Direct mail sync — runs syncChannelMail synchronously in the request handler.
...
import { syncChannelMail } from '../../../workers/mail-sync'
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (state.capabilities?.mail?.enabled !== true) {
      return NextResponse.json(
        { error: 'Email sync is not enabled for this channel' },
        { status: 409 },
      )
    }
    ...
    console.error('[channel_office365] mail-sync-now.post failed', error)
    return NextResponse.json(
      { error: translate('channel_office365.errors.mailSyncFailed', 'Failed to run mail sync') },
      { status: 500 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      summary: 'Run email sync immediately (direct, no queue) for a connected Office 365 channel',
